...
// Current -> Target (Original Original)
const t1 = makeTransformer(71.614, 225.491, 64.786, 222.077);
const t2 = makeTransformer(66.470, 217.988, 59.530, 214.518);
const t3 = makeTransformer(54.160, 226.948, 38.641, 219.188);

const finalTower1 = tower1Data.map(f => Object.assign({}, f, { d: transformPath(f.d, t1) }));
let finalTower2 = tower2Data.map(f => Object.assign({}, f, { d: transformPath(f.d, t2) }));
const finalTower3 = tower3Data.map(f => Object.assign({}, f, { d: transformPath(f.d, t3) }));

// Add missing floors for Tower 2
// The average floor height in restored T2: (214.518 - 59.530) / 47 = 3.2976
const t2AvgHeight = (214.518 - 59.530) / 47;
const shiftPathDown = (d, dy) => {
  const shiftFn = y => y + dy;
  shiftFn.scale = dy2 => dy2;
  return transformPath(d, shiftFn);
};

const floor3 = finalTower2.find(f => f.floor === '03th Floor');
if (floor3) {
  const floor2 = Object.assign({}, floor3, { floor: '02th Floor', d: shiftPathDown(floor3.d, t2AvgHeight) });
  const floor1 = Object.assign({}, floor3, { floor: '01th Floor', d: shiftPathDown(floor3.d, t2AvgHeight * 2) });
  finalTower2.push(floor2, floor1);
}
...
console.log('Successfully restored and fixed towerData.js');
